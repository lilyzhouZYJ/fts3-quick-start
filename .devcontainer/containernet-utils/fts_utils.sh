...
init_fts () {
    ...
    docker-compose -p $COMPOSE_PROJECT exec $RUCIO_NODE fts-rest-whoami -v -s https://$FTS_HOST:8446
    docker-compose -p $COMPOSE_PROJECT exec $RUCIO_NODE fts-rest-delegate -vf -s https://$FTS_HOST:8446 -H 9999
}
...
submit_test_transfers () {
    ...
    local dest_se=$2
    local filename=$3
    docker-compose -p $COMPOSE_PROJECT exec $RUCIO_NODE fts-rest-transfer-submit -v -s https://$FTS_HOST:8446 root://$source_se//rucio/$filename root://$dest_se//rucio/$filename
}
...
config_fts_link () {
    ...
    docker-compose -p $COMPOSE_PROJECT exec $RUCIO_NODE curl --capath /etc/grid-security/certificates -E /tmp/x509up_u0 --cacert /tmp/x509up_u0 -H "Content-Type: application/json" -d "$msg" https://fts:8446/config/links
    set +f
}
...
config_optimizer () {
    ...
    docker-compose -p $COMPOSE_PROJECT exec $RUCIO_NODE curl --capath /etc/grid-security/certificates -E /tmp/x509up_u0 --cacert /tmp/x509up_u0 -H "Content-Type: application/json" -d "$msg" https://fts:8446/optimizer/current
}
